...
function dotColor(step: StepView): string {
  if (step.state !== "completed") {
    if (step.state === "active") return "bg-primary";
    if (step.state === "skipped") return "bg-gray-100 border border-gray-300";
    return "bg-primary";
  }
  if (!step.decision?.label) return "bg-primary";
...
function getRailColor(step: StepView): string {
  ...
  if (!step.decision?.label) return "bg-primary/30";
...
function Badge({ state, decision }: { state: "completed" | "active" | "pending" | "skipped"; decision?: { label: string } | null }) {
  ...
  if (state === "active") {
    return <span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-primary/10 text-primary inline-flex items-center gap-1 whitespace-nowrap">In Progress</span>;
  }
  if (state === "skipped") {
...
function WaitingDetails({ actor }: { actor: string }) {
  return (
    <>
      <p className="text-sm text-primary font-semibold mb-3">Awaiting action from <strong>{actor}</strong></p>
      <PendingDetails />
    </>
  );
}
  ...
  return (
    <div className="detail-panel-shell h-full">
      ...
      <div className="flex items-center gap-3 mb-6">
        <div className="w-[34px] h-[34px] rounded-[10px] bg-gradient-to-br from-primary to-purple-600 flex items-center justify-center flex-shrink-0">
        ...
        <h1 className="inline-flex rounded-full border border-primary/20 bg-primary/5 px-4 py-1.5 text-sm font-extrabold uppercase tracking-[0.2em] text-primary shadow-sm m-0">Approval Workflow</h1>
      </div>
              ...
              <div className="flex flex-col items-center flex-shrink-0">
                ...
                <div className="bg-gray-50 border border-gray-200 rounded-[14px] p-4">
                  ...
                  {step.state === "active" && onDecision && (
                    <>
                      <p className="text-xs font-semibold text-primary mb-0.5">Status</p>
                      <p className="text-sm font-semibold text-primary mb-3">Waiting for approval from {step.actor}</p>
                            ...
                            <div className="flex-shrink-0 mt-0.5">
                              <div
                                className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                                  decision === opt.value ? "border-primary" : "border-gray-300"
                                }`}
                              >
                                {decision === opt.value && (
      ...
      {onSubmit && (
        <div className="mt-5 bg-primary/5 rounded-[14px] p-4 text-center">
          ...
          <button
            ...
            className="w-full py-3 border-none rounded-[10px] bg-gradient-to-br from-primary to-purple-600 text-white text-sm font-bold cursor-pointer disabled:opacity-45 disabled:cursor-not-allowed"
          >
